...
Node* Next(Node* me) {
	//если у вершины нет правого поддерева и она является левым ребенком своего родителя - мы переходим на родителя
	if (!me->right && me->parent && me->parent->left == me) {
		return me->parent;
	}
	//если у вершины нет правого поддерева и она является правым ребенком своего родителя -
	// вернуться по родителям назад пока значение не станет больше (если не нашлось - вернуть 0)
	if (!me->right && me->parent && me->parent->right == me) {
		int saved_value = me->value;
		while (me->parent) {
			me = me->parent;
			if (me->value >= saved_value) {
				return me;
			}
		}
		return nullptr;
	...
	//если у вершины есть правое поддерево - мы переходим на него, а потом до конца влево
	if (me->right) {
		me = me->right;
		while (me->left) {
			me = me->left;
